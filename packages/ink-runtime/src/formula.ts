...
import type { Formula, MathLang } from "../../types/src/ast";
...
async function greedyDecode(
  ...
  for (let step = 0; step < maxLen; step++) {
    ...
    let best = -Infinity, next = 0;
    for (let v = 0; v < vocabSz; v++) {
      if (logits[offset + v] > best) { best = logits[offset + v]; next = v; }
    }
...
export function toFormulaNode(latex: string, lang: MathLang = "latex"): Formula {
  return { type: "formula", lang, value: latex };
}

export async function recognizeFormula(imgBuffer: Buffer): Promise<Formula> {
  return toFormulaNode(await formulaToLatex(imgBuffer));
}
